...
program
  ...
  .version('1.0.0')
  .requiredOption(
    '-a, --sortingGroup <learningSource>',
    'The learning source that aligns with the kanji list provided (e.g. jlpt n5, rtk, wanikani)',
  )
  .option(
    '-f, --filename <name>',
    'The base kanji list filename located in data/raw/kanji-set',
    'kanji-set.txt',
  )
  .option(
    '-m, --maxWords <amount>',
    'Max number of example words',
...
async function runPipeline() {
  try {
    ...
    const externalKanjiDataString = await readRawFile(
      PATHS.kanjiDataSourcePath,
    );
    ...
    const spokenFrequenciesString = await readRawFile(
      PATHS.spokenFrequenciesSourcePath,
    );
    const spokenFrequenciesObject = parseJsonToObject(spokenFrequenciesString);
    const spokenFrequenciesMap = buildWordsFrequencyMap(
      spokenFrequenciesObject,
    );
    const tubelexOccurrenceMap = await buildTubelexOccurrencesMap(
      PATHS.tubelexOccurrencesSourcePath,
    );
    const literaryFrequenciesString = await readRawFile(
      PATHS.literaryFrequenciesSourcePath,
    );
    const literaryFrequenciesObject = parseJsonToObject(
      literaryFrequenciesString,
    );
    const literaryFrequenciesMap = buildWordsFrequencyMap(
      literaryFrequenciesObject,
    );
    mainDataSet = enrichWordsWithFrequency(
      mainDataSet,
      spokenFrequenciesMap,
      tubelexOccurrenceMap,
      literaryFrequenciesMap,
    );

    mainDataSet = sortWordsByPriority(mainDataSet);
    mainDataSet = maxOutWords(mainDataSet, options);
    mainDataSet = enrichKanjiWithSortingGroup(
      mainDataSet,
      options.sortingGroup,
    );
    mainDataSet = enrichWithTargetKanjiReadingType(mainDataSet);
    mainDataSet = await replaceReadingTypes(
      mainDataSet,
      PATHS.rawDataDirectoryPath,
    );
    await reportMissingReadingTypes(mainDataSet, PATHS.jsonOutputDirectory);

    const svgDataString = await readRawFile(
      join(PATHS.rawDataDirectoryPath, 'kanjivg-master.json'),
    );
    ...
    await buildTsv(
      jsonFileName,
      PATHS.jsonOutputDirectory,
      PATHS.tsvOutputDirectory,
      options.sortingGroup,
    );
